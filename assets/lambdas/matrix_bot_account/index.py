...
def handler(event, _ctx):
    ...
    if request_type == "Delete":
        return {
            "PhysicalResourceId": event.get("PhysicalResourceId", "matrix-bot-account")
        }

    if _current_token():
        return {"PhysicalResourceId": "matrix-bot-account"}
    ...
    exit_code = next(
        (c.get("exitCode") for c in containers if c.get("name") == CONTAINER_NAME),
        None,
    )
    if exit_code != 0:
        raise RuntimeError(f"matrix bot task exited with code {exit_code}: {task_arn}")

    output = _fetch_log_output(task_arn, log_stream_prefix, log_group)
    # The task's last line is a single JSON object with token/user_id/device_id.
    payload = None
    ...
    if not payload:
        raise RuntimeError(
            f"could not extract bot account JSON from task output: {output!r}"
        )
